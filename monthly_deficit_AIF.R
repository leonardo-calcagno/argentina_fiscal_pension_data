...
df_public_firms_deficit<-df_AIF %>% 
  mutate(is_public_firm_deficit=ifelse(grepl("*DEFICIT OPER",concepto) | grepl("D…FICIT OPER",concepto), 1, 
                                       0)
  )%>% 
...
df_public_firms_surplus<-df_AIF %>% 
  mutate(is_public_firm_surplus=ifelse(grepl("*SUPERAVIT OPER",concepto) | grepl("SUPER¡VIT OPER",concepto), 1, 
                                0)
        ) %>%  
...
setwd("C:/Users/lcalcagno/Documents/i√≥n/MISSAR_private/R_files_for_MISSAR/")
